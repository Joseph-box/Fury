...
local function Fury_Configuration_Init()
	...
	if not Fury_ImmuneInterrupt then
		Fury_ImmuneInterrupt = {}
	end
...
local function DoShapeShift(stance)
	local stances = {
		ABILITY_BATTLE_STANCE_FURY,
		ABILITY_DEFENSIVE_STANCE_FURY,
		ABILITY_BERSERKER_STANCE_FURY,
	}
...
local res = {
	...
	["frost"] = { BOSS_NAX_KEL_THUZAD_FURY, BOSS_NAX_SAPPHIRON_FURY },
	["nature"] = {
...
local function Fury_InitDistance()
	...
	for i = 1, 120 do
		...
		if t then
			if not yard30 then
				if string.find(t, "Ability_Marksmanship") or string.find(t, "Ability_Throw") then -- Shoot -- Throw
					yard30 = i
					Debug("30 yard: " .. t)
					found = found + 1
				end
			...
			if not yard25 then
				if string.find(t, "Ability_Warrior_Charge") or string.find(t, "Ability_Rogue_Sprint") then -- Charge -- Intercept
					yard25 = i
					Debug("25 yard: " .. t)
					found = found + 1
				end
			...
			if not yard05 then
				if
					string.find(t, "Ability_Warrior_Sunder") -- Sunder Armor
					...
					or string.find(t, "Spell_Nature_Bloodlust")
				then -- Bloodthirst
					yard05 = i
					Debug("5 yard: " .. t)
					found = found + 1
	...
	if FuryOldStance == GetActiveStance() then
		Debug(
			"Fury Dance Done ("
				.. text
				.. ") - Old: "
				.. tostring(FuryOldStance)
				.. " Current:"
				.. tostring(GetActiveStance())
		)
		FuryDanceDone = true
	end
end
...
local function HasWeapon()
	...
	if item then
		...
		if
			itemLink ~= "item:7005:0:0:0" -- Skining knife
			and itemLink ~= "item:2901:0:0:0" -- Mining pick
			and not GetInventoryItemBroken("player", 16)
		then
			return true
		end
	end
...
local function HasImmobilizingDebuff()
	return CheckDebuffs("player", { "Spell_Frost_FrostNova", "spell_Nature_StrangleVines" })
end
--------------------------------------------------
...
local function Fury_ImmuneInterruptDetect(arg1, arg2)
	-- Thanks to Harrio
	if arg1 == CHAT_INTERRUPT_IMMUNE_FURY then
		Fury_ImmuneInterrupt[arg2] = true
	end
end
--------------------------------------------------
...
local function AddEnemyCount(Enemies)
	...
	if
		Fury_Configuration["AutoDisableAoe"]
...
local function Fury_TreatDebuffPlayer()
	...
	if UnitName("target") == BOSS_NAX_LOATHEB_FURY or UnitName("target") == BOSS_NAX_SAPPHIRON_FURY then
	...
	if HasDebuffType("player", ITEM_DEBUFF_TYPE_POISON) then
		...
		elseif
			UnitRace("player") == RACE_DWARF
			and Fury_Configuration[RACIAL_STONEFORM_FURY]
			and IsSpellReady(RACIAL_STONEFORM_FURY)
		then
...
function Fury()
	...
	then
		...
		if Fury_Configuration["PrimaryStance"] and Fury_Configuration["PrimaryStance"] ~= GetActiveStance() then
		...
		if
			Fury_Configuration["ShieldBashSwapped"]
			and (not FurySpellInterrupt or not IsSpellReady(ABILITY_SHIELD_BASH_FURY))
		...
		if FuryMount then
			...
			FuryMount = nil
		elseif
			Fury_Configuration[ABILITY_BERSERKER_RAGE_FURY]
			and (FuryIncapacitate or FuryFear)
			and GetActiveStance() == 3
			and IsSpellReady(ABILITY_BERSERKER_RAGE_FURY)
		then
			-- 7, Spider Belt, remove existing immobilizing effects
			Debug("6. Berserker Rage")
			CastSpellByName(ABILITY_BERSERKER_RAGE_FURY)
		elseif debuffImmobilizing and IsEquippedAndReady(6, ITEM_BELT_SPIDER_BELT_FURY) then
			-- 8, Ornate Mithril Boots, remove existing immobilizing effects
			Debug("7. Spider Belt")
			UseInventoryItem(6)
		elseif debuffImmobilizing and IsEquippedAndReady(8, ITEM_BOOTS_ORNATE_MITHRIL_BOOTS_FURY) then
			-- 9, PVP Trinket, Horde
			Debug("8. Ornate Mithril Boots")
			UseInventoryItem(8)
		elseif
			(FuryFear or FuryIncapacitate or debuffImmobilizing)
			and IsTrinketEquipped(ITEM_TRINKET_INSIGNIA_OF_THE_HORDE_FURY)
		then
		...
		elseif
			(FuryFear or FuryIncapacitate or debuffImmobilizing)
			and IsTrinketEquipped(ITEM_TRINKET_INSIGNIA_OF_THE_ALLIANCE_FURY)
		then
			...
			UseInventoryItem(slot)
		elseif
			Fury_Configuration[ABILITY_EXECUTE_FURY]
			and HasWeapon()
			and not Fury_Configuration[MODE_HEADER_AOE]
			and UnitRage(ABILITY_EXECUTE_FURY)
			and (GetActiveStance() ~= 2 or (Fury_Configuration["PrimaryStance"] ~= 2 and UnitUnderRageStanceCap() and Fury_Configuration["PrimaryStance"] ~= 0))
			and (UnitHealth("target") / UnitHealthMax("target") * 100) <= 20
			and IsSpellReady(ABILITY_EXECUTE_FURY)
		then
			-- Overpower when available
			-- Swap to Execute weapon
			if
				Fury_Configuration["ExecuteSwap"]
			...
			FuryLastSpellCast = GetTime()
		elseif
			Fury_Configuration[ABILITY_OVERPOWER_FURY]
			and FuryOverpower
			and HasWeapon()
			and not Fury_Configuration[MODE_HEADER_AOE]
			and UnitRage(ABILITY_OVERPOWER_FURY)
			and (
				GetActiveStance() == 1 -- if already in battle stance
				or (
					(
						(
							Fury_Configuration["PrimaryStance"] ~= 2 -- or another stance that isnt prot if:
			...
			and IsSpellReady(ABILITY_OVERPOWER_FURY)
		then
			-- Pummel if casting
			...
			FuryLastSpellCast = GetTime()
		elseif
			Fury_Configuration[ABILITY_PUMMEL_FURY]
			and FurySpellInterrupt
			and UnitRage(ABILITY_PUMMEL_FURY)
			and (not UnitIsPlayer("target") or (UnitIsPlayer("target") and (UnitClass("target") ~= CLASS_ROGUE_FURY and UnitClass(
				"target"
			) ~= CLASS_WARRIOR_FURY and UnitClass("target") ~= CLASS_HUNTER_FURY)))
			and (GetActiveStance() == 3 or (not (HasShield() or (Fury_HasShieldInBags() and Fury_Configuration["ShieldBashSwap"])) and UnitUnderRageStanceCap() and Fury_Configuration["PrimaryStance"] ~= 0))
			and IsSpellReady(ABILITY_PUMMEL_FURY)
			and not FURY_INTERRUPT_BLACKLIST[UnitName("target")]
			and not Fury_ImmuneInterrupt[UnitName("target")]
		then
		...
		elseif
			Fury_Configuration[ABILITY_SHIELD_BASH_FURY]
			and FurySpellInterrupt --   and not Fury_Configuration[MODE_HEADER_AOE]
			and UnitRage(ABILITY_SHIELD_BASH_FURY)
			and (HasShield() or (Fury_HasShieldInBags() and Fury_Configuration["ShieldBashSwap"]))
			and (not UnitIsPlayer("target") or (UnitIsPlayer("target") and (UnitClass("target") ~= CLASS_ROGUE_FURY and UnitClass(
				"target"
			) ~= CLASS_WARRIOR_FURY and UnitClass("target") ~= CLASS_HUNTER_FURY)))
			and GetActiveStance() ~= 3
		...
		then
			...
			-- Equip shield
			if not HasShield() and not Fury_Configuration["ShieldBashSwapped"] and Outfitter_ExecuteCommand then
		...
		elseif
			Fury_Configuration[ABILITY_HAMSTRING_FURY]
			and (UnitIsPlayer("target") or (Fury_Runners[UnitName("target")] and (UnitHealth("target") / UnitHealthMax(
				"target"
			) * 100) <= tonumber(Fury_Configuration["HamstringHealth"])))
			and HasWeapon()
			...
			and (GetActiveStance() ~= 2 or (UnitUnderRageStanceCap() and Fury_Configuration["PrimaryStance"] ~= 0))
			and IsSpellReady(ABILITY_HAMSTRING_FURY)
		then
			-- Rend to antistealth
		...
		elseif
			Fury_Configuration[ABILITY_REND_FURY]
			...
			and (GetActiveStance() ~= 3 or (UnitUnderRageStanceCap() and Fury_Configuration["PrimaryStance"] ~= 0))
			and IsSpellReady(ABILITY_REND_FURY)
		then
			-- slow target
		...
		elseif
			Fury_Configuration[ABILITY_PIERCING_HOWL_FURY]
			...
			and IsSpellReady(ABILITY_PIERCING_HOWL_FURY)
		then
			-- Rooted
			Debug("17. Piercing Howl")
			CastSpellByName(ABILITY_PIERCING_HOWL_FURY)
			FuryLastSpellCast = GetTime()
		elseif debuffImmobilizing and Fury_Distance() >= 8 then
			-- Berserker rage
			if GetActiveStance() ~= 2 then
			...
			if slot ~= nil and (FurySpellInterrupt or UnitClass("target") == CLASS_HUNTER_FURY) then
				Debug("18. Linken's Boomerang")
				UseInventoryItem(slot)
			else
				slot = IsTrinketEquipped(ITEM_TRINKET_TIDAL_CHARM)
				if slot ~= nil and FurySpellInterrupt then
		...
		elseif
			...
			and (
				...
				or (
					...
					and not UnitRage(ABILITY_BERSERKER_RAGE_FURY) --   and UnitMana("player") <= FuryTacticalMastery
					and Fury_Configuration["PrimaryStance"] ~= 0
				)
			)
			and IsSpellReady(ABILITY_BERSERKER_RAGE_FURY)
		then
			...
			CastSpellByName(ABILITY_BERSERKER_RAGE_FURY)
		elseif
			Fury_Configuration["PrimaryStance"] -- if there is no primary stance there is no dance
			...
			and ((FuryLastStanceCast and FuryLastStanceCast + 1 <= GetTime()) or not FuryLastStanceCast)
			and UnitUnderRageStanceCap()
		then
			-- Disarm (PVP only)
			...
			FuryDanceDone = true
		elseif
			Fury_Configuration[ABILITY_DISARM_FURY]
			and HasWeapon()
			and UnitIsPlayer("target")
			and (UnitClass("target") == CLASS_HUNTER_FURY or UnitClass("target") == CLASS_PALADIN_FURY or UnitClass(
				"target"
			) == CLASS_ROGUE_FURY or UnitClass("target") == CLASS_SHAMAN_FURY or UnitClass("target") == CLASS_WARRIOR_FURY)
			and UnitRage(ABILITY_DISARM_FURY)
			and Fury_ImmuneDisarm[UnitName("target")] == nil
			and (GetActiveStance() == 2 or (UnitUnderRageStanceCap() and Fury_Configuration["PrimaryStance"] ~= 0))
			and IsSpellReady(ABILITY_DISARM_FURY)
		then
			-- Sweeping Strikes
			if GetActiveStance() ~= 2 then
		...
		elseif
			...
			and IsSpellReady(ABILITY_SWEEPING_STRIKES_FURY)
		then
			-- Bloodthirst
		...
		elseif
			...
			and (
				not Fury_Configuration[MODE_HEADER_AOE]
				or UnitMana("player") >= (Fury_Configuration["MaximumRage"] + Fury_Configuration["NextAttackRage"])
			) -- test to dump with BT
			and UnitRage(ABILITY_BLOODTHIRST_FURY)
			and IsSpellReady(ABILITY_BLOODTHIRST_FURY)
		then
			-- Mortal Strike
		...
		elseif
			...
			and IsSpellReady(ABILITY_MORTAL_STRIKE_FURY)
		then
			-- Whirlwind
		...
		elseif
			(
				Fury_Configuration[ABILITY_WHIRLWIND_FURY] and not IsSpellReady(ABILITY_BLOODTHIRST_FURY)
				or Fury_Configuration[MODE_HEADER_AOE]
			)
			and Fury_Distance() <= 10
			and HasWeapon()
			and UnitRage(ABILITY_WHIRLWIND_FURY)
			and (GetActiveStance() == 3 or (Fury_Configuration["PrimaryStance"] ~= 2 and UnitUnderRageStanceCap() and Fury_Configuration["PrimaryStance"] ~= 0))
			and IsSpellReady(ABILITY_WHIRLWIND_FURY)
		then
			-- Shield Slam
			if GetActiveStance() ~= 3 then
		...
		elseif
			Fury_Configuration[ABILITY_SHIELD_SLAM_FURY]
			and FuryShieldSlam
			and HasShield()
			and UnitRage(ABILITY_SHIELD_SLAM_FURY)
			and IsSpellReady(ABILITY_SHIELD_SLAM_FURY)
		then
		...
		then
			...
			FuryLastSpellCast = GetTime()
		elseif
			Fury_Configuration[ABILITY_DEMORALIZING_SHOUT_FURY]
			...
			and (not UnitIsPlayer("target") or UnitClass("target") == CLASS_WARRIOR_FURY or UnitClass("target") == CLASS_ROGUE_FURY)
			and UnitLevel("Player") - UnitLevel("Target") < Fury_Configuration["DemoDiff"]
			and FuryAttack == true
			and IsSpellReady(ABILITY_DEMORALIZING_SHOUT_FURY)
		then
			-- Revenge
		...
		then
			...
			FuryLastSpellCast = GetTime()
		elseif
			Fury_Configuration[ABILITY_SHIELD_BLOCK_FURY]
			and HasShield()
			and FuryCombat
			and GetActiveStance() == 2
		...
		then
			...
			if FuryOldStance == GetActiveStance() or Fury_Configuration["PrimaryStance"] == GetActiveStance() then
		...
		elseif
			...
			and FuryAttack == true
			and IsItemReady(ITEM_CONS_JUJU_FLURRY)
		then
			-- Juju Chill
		...
		elseif
			...
			and IsUseRes("frost")
			and IsItemReady(ITEM_CONS_JUJU_CHILL)
		then
			-- Juju Ember
		...
		elseif
			...
			and IsUseRes("fire")
			and IsItemReady(ITEM_CONS_JUJU_EMBER)
		then
			-- Juju Might
		...
		elseif
			...
			and IsItemReady(ITEM_CONS_JUJU_MIGHT)
		then
			-- Juju Power
		...
		elseif
			...
			and IsItemReady(ITEM_CONS_JUJU_POWER)
		then
			-- Immolation potion
		...
		elseif
			...
			and IsSpellReady(RACIAL_BERSERKING_FURY)
		then
		...
		elseif
			...
			and GetActiveStance() ~= 2
			and FuryCombat
			and (UnitHealth("player") / UnitHealthMax("player") * 100) >= tonumber(
				Fury_Configuration["DeathWishHealth"]
			)
			and IsSpellReady(RACIAL_BLOOD_FURY)
		then
			-- Death Wish
			Debug("41. Blood Fury")
		...
		elseif
			...
			and IsSpellReady(ABILITY_DEATH_WISH_FURY)
		then
			-- Earthstrike
			Debug("42. Death Wish")
			CastSpellByName(ABILITY_DEATH_WISH_FURY)
		elseif
			Fury_Configuration[ITEM_TRINKET_EARTHSTRIKE]
			and FuryCombat
			and FuryAttack == true
			and IsTrinketEquipped(ITEM_TRINKET_EARTHSTRIKE)
		then
			-- Slayer's Crest
			Debug("43. Earthstrike")
			UseInventoryItem(IsTrinketEquipped(ITEM_TRINKET_EARTHSTRIKE))
		elseif
			Fury_Configuration[ITEM_TRINKET_SLAYERS_CREST]
			and FuryCombat
			and FuryAttack == true
			and IsTrinketEquipped(ITEM_TRINKET_SLAYERS_CREST)
		then
			-- Kiss of the Spider
			Debug("44. Slayer's Crest")
			UseInventoryItem(IsTrinketEquipped(ITEM_TRINKET_SLAYERS_CREST))
		elseif
			Fury_Configuration[ITEM_TRINKET_KOTS]
			and FuryCombat
			and FuryAttack == true
			and IsTrinketEquipped(ITEM_TRINKET_KOTS)
		then
			-- Bloodrage
			Debug("45. Kiss of the Spider")
			UseInventoryItem(IsTrinketEquipped(ITEM_TRINKET_KOTS))
		elseif
			Fury_Configuration[ABILITY_BLOODRAGE_FURY]
			and UnitMana("player") <= tonumber(Fury_Configuration["MaximumRage"])
			and (UnitHealth("player") / UnitHealthMax("player") * 100) >= tonumber(
				Fury_Configuration["BloodrageHealth"]
			)
			and IsSpellReady(ABILITY_BLOODRAGE_FURY)
		then
			-- Treat debuffs (poisons)
			Debug("46. Bloodrage")
			CastSpellByName(ABILITY_BLOODRAGE_FURY)
		elseif Fury_Configuration[MODE_HEADER_DEBUFF] and Fury_TreatDebuffPlayer() then
			-- Dump rage with Heroic Strike or Cleave
			Debug("47. Treated debuff")
		elseif
			(HasWeapon())
			and (
				Fury_Configuration[MODE_HEADER_AOE]
				or Fury_Configuration["PrimaryStance"] == 2
				or (
						(
							Fury_Configuration[ABILITY_MORTAL_STRIKE_FURY]
					...
					and ((Fury_Configuration[ABILITY_BLOODTHIRST_FURY] and FuryBloodthirst and not IsSpellReady(
						ABILITY_BLOODTHIRST_FURY
					)) or not Fury_Configuration[ABILITY_BLOODTHIRST_FURY] or not FuryBloodthirst)
					and ((Fury_Configuration[ABILITY_WHIRLWIND_FURY] and not IsSpellReady(ABILITY_WHIRLWIND_FURY)) or not Fury_Configuration[ABILITY_WHIRLWIND_FURY] or not FuryWhirlwind)
			)
		then
			...
			if
				Fury_Configuration[ABILITY_HAMSTRING_FURY]
				and not Fury_Configuration[MODE_HEADER_FURY]
				and UnitRage(ABILITY_HAMSTRING_FURY)
				and UnitMana("player") >= tonumber(Fury_Configuration["FlurryTriggerRage"])
				and ((FuryFlurry and not HasBuff("player", "Ability_GhoulFrenzy")) or FuryImpHamstring or FurySwordSpec or FuryMaceSpec)
				and IsSpellReady(ABILITY_HAMSTRING_FURY)
			then
				-- Heroic Strike
				-- Try trigger...
				...
				FuryLastSpellCast = GetTime()
			elseif
				Fury_Configuration[ABILITY_HEROIC_STRIKE_FURY]
				and not Fury_Configuration[MODE_HEADER_AOE]
				and UnitRage(ABILITY_HEROIC_STRIKE_FURY)
				and (UnitMana("player") >= tonumber(Fury_Configuration["NextAttackRage"]) or (not FuryMortalStrike and not FuryWhirlwind and not FuryBloodthirst) or Fury_Configuration["PrimaryStance"] == 2)
				and IsSpellReady(ABILITY_HEROIC_STRIKE_FURY)
			then
				...
				FuryLastSpellCast = GetTime()
			elseif
				Fury_Configuration[ABILITY_CLEAVE_FURY]
				or Fury_Configuration[MODE_HEADER_AOE]
					and UnitRage(ABILITY_CLEAVE_FURY)
					and ((UnitMana("player") >= tonumber(Fury_Configuration["NextAttackRage"])) or (Fury_Configuration[MODE_HEADER_AOE] and UnitMana(
						"player"
					) >= 25) or Fury_Configuration["PrimaryStance"] == 2)
					and IsSpellReady(ABILITY_CLEAVE_FURY)
			then
		...
		if
			Fury_Configuration["ShieldBashSwapped"]
			and Outfitter_ExecuteCommand
			and not (FurySpellInterrupt or IsSpellReady(ABILITY_SHIELD_BASH_FURY))
		then
			...
			Fury_Configuration["ShieldBashSwapped"] = false
		elseif
			Fury_Configuration["ExecuteSwapped"]
			and Outfitter_ExecuteCommand
			and (UnitHealth("target") / UnitHealthMax("target") * 100 > 21 or UnitIsDead("target") or not FuryCombat)
		then
...
local function Fury_Charge()
	...
	if not UnitExists("target") and not FuryCombat then
		if
			Fury_Configuration["PrimaryStance"]
			and Fury_Configuration["PrimaryStance"] ~= 0
			and GetActiveStance() ~= Fury_Configuration["PrimaryStance"]
		then
			DoShapeShift(Fury_Configuration["PrimaryStance"])
		end
	...
	if FuryCombat then
		...
		elseif
			Fury_Configuration[ABILITY_INTERCEPT_FURY]
			and GetActiveStance() == 3
			and dist <= 25
			and dist > 7
		...
		elseif
			...
			and IsSpellReadyIn(ABILITY_INTERCEPT_FURY) <= 3
		then
		...
		if
			Fury_Configuration[ABILITY_CHARGE_FURY]
			and GetActiveStance() == 1
			and dist <= 25
			and dist > 7
			and FuryLastChargeCast + 0.5 < GetTime()
			and IsSpellReady(ABILITY_CHARGE_FURY)
		then
			Debug("O1. Charge")
...
--------------------------------------------------
local armsmode = { { "PrimaryStance", 1 } }

local tankmode = {
	{ "PrimaryStance", 2 }, --  {"ExecuteSwap", false},
...
local furymode = { { "PrimaryStance", 3 } }
			...
			fn = function(options)
				...
				FuryPrint(
					TEXT_FURY_NAME
						...
						.. TEXT_FURY_CLASS
						.. (UnitClass(target) or "")
						.. TEXT_FURY_CLASSIFICATION
						.. (UnitClassification(target) or "")
				)
...
function Fury_OnEvent(event)
	...
	elseif
		(
			event == "CHAT_MSG_COMBAT_SELF_MISSES"
			or event == "CHAT_MSG_SPELL_SELF_DAMAGE"
			or event == "CHAT_MSG_SPELL_DAMAGESHIELDS_ON_SELF"
		) and (string.find(arg1, CHAT_OVERPOWER1_FURY) or string.find(arg1, CHAT_OVERPOWER2_FURY))
	then
	...
	elseif
		...
		and (
			string.find(arg1, CHAT_OVERPOWER3_FURY)
			or string.find(arg1, CHAT_OVERPOWER4_FURY)
			or string.find(arg1, CHAT_OVERPOWER5_FURY)
		)
	then
	...
	elseif
		event == "CHAT_MSG_SPELL_SELF_DAMAGE" and string.find(arg1, CHAT_INTERRUPT1_FURY)
		or event == "CHAT_MSG_COMBAT_SELF_MISSES"
			and (string.find(arg1, CHAT_INTERRUPT2_FURY) or string.find(arg1, CHAT_INTERRUPT3_FURY) or string.find(
				arg1,
				CHAT_INTERRUPT4_FURY
			) or string.find(arg1, CHAT_INTERRUPT5_FURY))
	then
	...
	elseif
		event == "CHAT_MSG_SPELL_SELF_DAMAGE"
		and (string.find(arg1, CHAT_INTERRUPT6_FURY) or string.find(arg1, CHAT_INTERRUPT7_FURY))
	then
		-- Check to see if Pummel/Shield target is immune
		-- FuryPrint(TEXT_FURY_ADDED .. name .. TEXT_FURY_IMMUNE_TO_INTERRUPT)
		FuryPrint("CHAT_MSG_SPELL_SELF_DAMAGE " .. arg1)
		FurySpellInterrupt = nil
	elseif
		event == "CHAT_MSG_SPELL_DAMAGESHIELDS_ON_SELF"
		and (string.find(arg1, CHAT_INTERRUPT6_FURY) or string.find(arg1, CHAT_INTERRUPT7_FURY))
	then
		-- Check to see if Pummel/Shield target is immune
		-- local _, _, name = string.find(arg1, CHAT_INTERRUPT6_FURY) || string.find(arg1, CHAT_INTERRUPT7_FURY)
		-- if name ~= nil then
		-- 	Fury_ImmuneInterrupt[name] = true
		-- 	FuryPrint(TEXT_FURY_ADDED .. name .. TEXT_FURY_IMMUNE_TO_INTERRUPT)
		-- end
		-- FuryPrint("CHAT_MSG_SPELL_DAMAGESHIELDS_ON_SELF " .. arg1)
		FurySpellInterrupt = nil
	elseif
		(event == "CHAT_MSG_SPELL_SELF_DAMAGE" or event == "CHAT_MSG_COMBAT_SELF_MISSES")
		and string.find(arg1, CHAT_WHIRLWIND_FURY)
	then
	...
	elseif
		(event == "CHAT_MSG_SPELL_SELF_DAMAGE" or event == "CHAT_MSG_COMBAT_SELF_MISSES")
		and string.find(arg1, CHAT_CLEAVE_FURY)
	then
	...
	elseif event == "CHAT_MSG_SPELL_PERIODIC_SELF_DAMAGE" then
		...
		elseif
			...
			or arg1 == CHAT_HOWL_OF_TERROR_FURY
		then
			FuryFear = true
		end
	elseif event == "CHAT_MSG_SPELL_AURA_GONE_SELF" then
		...
		if
			...
			or arg1 == CHAT_ROCKET_HELM2_FURY
		then
			FuryIncapacitate = nil
		elseif
			arg1 == CHAT_FEAR2_FURY
			...
			or arg1 == CHAT_HOWL_OF_TERROR2_FURY
		then
			FuryFear = nil
	...
	elseif event == "CHAT_MSG_COMBAT_CREATURE_VS_SELF_MISSES" then
		...
		if
			string.find(arg1, CHAT_BLOCK_FURY)
			or string.find(arg1, CHAT_PARRY_FURY)
			or string.find(arg1, CHAT_DODGE_FURY)
		then
	...
	elseif event == "PLAYER_TARGET_CHANGED" then
		...
		FuryFleeing = nil
		FurySpellInterrupt = nil
		if not FuryTalents and UnitClass("player") == CLASS_WARRIOR_FURY then
	...
	elseif
		(event == "CHARACTER_POINTS_CHANGED" and arg1 == -1)
		or event == "PLAYER_LEVEL_UP"
		or event == "PLAYER_ENTERING_WORLD" and UnitClass("player") == CLASS_WARRIOR_FURY
	then
		if Fury_Configuration["DebugChannel"] then
			LogToFile(true)
		end
	...
	elseif event == "CHAT_MSG_SPELL_DAMAGESHIELDS_ON_SELF" then
		...
		if name ~= nil then
			...
			FuryPrint(TEXT_FURY_ADDED .. name .. TEXT_FURY_IMMUNE_TO_DISARM)
		end
	end
end
